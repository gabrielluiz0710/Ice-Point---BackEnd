...
export class ProductsService {
    ...
    try {
      ...
      const newProduct = this.productRepo.create({
        ...
        categoria: categoryToSave
          ? categoryToSave
          : categoryId
            ? ({ id: Number(categoryId) } as Category)
            : undefined,
        informacaoNutricional:
          typeof createProductDto.informacaoNutricional === 'string'
      ...
      if (files && files.length > 0) {
        for (const [index, file] of files.entries()) {
          ...
          if (error)
            throw new InternalServerErrorException('Erro no upload da imagem');
  ...
  async update(
    ...
    try {
      ...
      let categoryToSave: Category | null = null;
      if (novaCategoria) {
        const existingCat = await this.categoryRepo.findOne({
          where: { nome: novaCategoria },
        });
        if (existingCat) {
        ...
        categoryToSave = await this.categoryRepo.findOne({
      ...
      if (disponivel !== undefined)
        updateData.disponivel = String(disponivel) === 'true';
      if (categoryToSave) updateData.categoria = categoryToSave;
      if (informacaoNutricional) {
        updateData.informacaoNutricional =
          typeof informacaoNutricional === 'string'
  ...
  async updatePriceByCategory(categoryId: number, newPrice: number) {
    ...
    return {
      message: `Preços da categoria ${category.nome} atualizados para R$ ${newPrice}`,
    };
  }
